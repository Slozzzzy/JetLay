...
const CreateAccountScreen: React.FC<CreateAccountProps> = ({ showScreen, showAlert, handleGoogleLogin }) => {
  ...
  const renderErrorBox = () => {
    ...
    return (
      <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg relative mb-6 text-left" role="alert">
        <div className="flex justify-between items-start">
          <strong className="font-bold">{isPasswordError ? 'Invalid Password' : 'Error'}</strong>
          {/* Close button */}
          <button onClick={() => setErrorMessage('')} className="text-xl font-bold leading-none -mt-1">
            &times;
          </button>
        </div>
    ...
    <div className="flex flex-col min-h-screen bg-purple-50">
      ...
      <div className="flex-1 p-6 flex justify-center items-start pt-12">
        <div className="bg-white p-10 rounded-2xl shadow-2xl w-full max-w-md text-center">
          ...
          <input type="text" placeholder="Full Name" className="w-full p-3 mb-4 border border-gray-300 rounded-lg" value={fullName} onChange={handleInputChange(setFullName)} />
          <input type="email" placeholder="Email Address" className="w-full p-3 mb-4 border border-gray-300 rounded-lg" value={email} onChange={handleInputChange(setEmail)} />
