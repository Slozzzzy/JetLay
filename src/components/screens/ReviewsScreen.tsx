...
const StarRow: React.FC<{
  ...
  size?: number;
  showNumber?: boolean;
}> = ({ value, size = 18, showNumber = false }) => {
  ...
  const reviews: Review[] = [
    {
      name: "Mmay M",
      stars: 5,
      text: "Bangkok → Paris trip. Visa approved in 5 days. Uploading documents to JETLAY was super smooth!",
      avatarBg: "#581c87",
      img: "Visa+Document+Preview",
      when: "2 weeks ago (demo)",
    },
    {
      name: "honny M",
      stars: 4,
      text: "JETLAY alerted me about my passport expiry before booking my Singapore trip. Lifesaver!",
      avatarBg: "#eab308",
      img: "Singapore+F1+Track+Preview",
      when: "1 month ago (demo)",
    },
    {
      name: "Fern B.",
      stars: 4,
      text: "Docs synced to Google Calendar perfectly! Reminder saved me.",
      avatarBg: "#6b21a8",
      img: null,
      when: "1 month ago (demo)",
    },
    {
      name: "Bank G.",
      stars: 3,
      text: "Visa took 10 days. Make sure to book flights early!",
      avatarBg: "#1e1b4b",
      img: null,
      when: "2 months ago (demo)",
    },
  ];
      ...
      <div className="p-6 flex-1 max-w-6xl mx-auto w-full text-center">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {reviews.map((review, index) => (
            <div
              key={index}
              className="bg-white rounded-xl p-5 shadow-lg border border-gray-200 flex flex-col gap-3">
              ...
              <span className="text-xs text-gray-400 mt-auto pt-2 text-left">
                {review.when ?? "—"}
              </span>
            </div>
          ))}
