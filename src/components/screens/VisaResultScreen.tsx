...
const VisaResultScreen: React.FC<ScreenProps> = ({
  showScreen,
  showAlert,
  profile,
}) => {
  const mockRequirements = [
    ...
    "Two recent passport-sized photos (35mm x 45mm)",
    ...
    <div className="flex flex-col min-h-screen bg-purple-50 pb-20">
      ...
      <div className="p-6 flex-1 max-w-3xl mx-auto w-full">
        <div className="bg-white p-6 rounded-xl shadow-xl border border-purple-200">
          ...
          <div className="space-y-3">
            {mockRequirements.map((item, index) => (
              <label
                key={index}
                className="flex items-center justify-between p-4 bg-white rounded-xl shadow-sm border border-gray-200 cursor-pointer hover:bg-gray-50">
        ...
        <div className="mt-6 space-y-3">
          <button
            className="cursor-pointer w-full py-3 text-white font-bold rounded-xl shadow-md"
            style={{
              background: "linear-gradient(90deg, #d8b4fe, #fbcfe8)",
              color: "#1e1b4b",
            }}
            onClick={() =>
              showAlert("Open official embassy link (demo)", "info")
            }>
            Open Official Source
          </button>
          <button
            className="cursor-pointer w-full py-3 bg-gray-900 text-white font-bold rounded-xl shadow-md hover:bg-gray-700"
